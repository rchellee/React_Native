...
export const getAdoptionRequests = async (userId) => {
  try {
    ...
    console.log('Fetched Adoption Requests:', requests.documents);
...
export const updateAdoptionRequest = async (requestId, data) => {
  ...
  try {
    console.log('Updating Adoption Request with ID:', requestId, 'Data:', data);
...
export const createRate = async ({ petId, userId, username, petName, rating, feedback, timestamp }) => {
  try {
    ...
    const adoptionRequestToUpdate = adoptionRequests.find(request => request.PetName === petName);
    if (adoptionRequestToUpdate) {
      
      await updateAdoptionRequest(adoptionRequestToUpdate.$id, { rated: "True" });
    } else {
    }
    return newRate;
  } catch (error) {
...
export const getPetIdByName = async (petName) => {
  try {
    ...
    console.error("Error fetching pet ID by name:", error);
    throw new Error(error.message);
  }
};
