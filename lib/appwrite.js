...
export const getAdoptionRequests = async (userId, asOwner, petName) => {
  try {
    const query = [];
    if (petName) {
      query.push(Query.equal("PetName", petName));
    }
...
export const createRate = async ({ petId, userId, username, petName, rating, feedback, timestamp }) => {
  try {
    ...
    const adoptionRequestToUpdate = adoptionRequests.find(request => request.petId === petId);
